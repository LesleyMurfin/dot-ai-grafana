...
`scripts/ci-local.sh` runs the [CI](.github/workflows/ci.yml) gate suite on your own machine, so a
full pass can be confirmed before pushing instead of after waiting on GitHub Actions.
...
### Gates that skip without Docker
