...
| I-021 | Document no-apply vs PR tokens | Document the credential split between the forever no-apply analysis token and separate GitOps PR-create (or bot) credentials. Analysis must keep working when execute credentials are absent or denied. Owned by `prds/2-gitops-pr-remediate.md`, not **PRD #5**. | `prds/2-gitops-pr-remediate.md:23,37` | PRD2 | OUT-OF-SCOPE (PRD #2) | — |
