...
#### Published precedent — and the honest negative

This class is documented; the specific channel is not. Both halves matter when the finding is written up.

| Source | What it establishes |
|--------|---------------------|
| [OWASP LLM01 — prompt injection](https://owasp.org/www-project-top-10-for-large-language-model-applications/) | The class, including indirect injection from content the system ingests. Cited above; nothing here is novel research. |
| **GrafanaGhost** — Noma Security, 2026-04-07, patched | Indirect prompt injection in Grafana's *own* AI assistant, exfiltrating via a markdown image beacon with no login and no click; its client-side image-URL validator was bypassed with a protocol-relative `//host` URL. Adjacent, same product family, same primitive — which is why S1's link-scheme check must reject protocol-relative URLs. No CVE. |
| **CamoLeak** — GitHub Copilot Chat, October 2025, CVSS 9.6 | The strongest precedent for S1's shape: GitHub's fix was to **disable image rendering** in chat output rather than filter it. Citation hygiene: this is widely mis-attributed to CVE-2025-59145, which is an unrelated npm `color-name` malware advisory — cite CamoLeak **by name, with no CVE**. |
| [grafana/mcp-grafana#680](https://github.com/grafana/mcp-grafana/issues/680) — open since 2026-03-24, unanswered | An upstream report already naming dashboard titles, panel descriptions, annotation text and Alertmanager incident descriptions as injection channels into a Grafana MCP surface. The concern is not ours alone, and it is not yet answered. |

**The honest negative:** a scan of all 57 MITRE ATLAS case studies (`mitre-atlas/atlas-data`, `dist/ATLAS.yaml`, 2026-09-04) found **zero** published cases whose injection channel is a log line, metric label, span attribute, or alert annotation. So this section claims a **class**, an adjacent patched case, and an unanswered upstream report — it does **not** claim a telemetry-channel incident exists. The absence is a reason to design now, not evidence of safety: I6 says the accident arrives without an adversary, and the only public precedent for a fix at this layer removed the rendering capability outright.
