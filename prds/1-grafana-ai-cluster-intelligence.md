...
|----|--------|---------|------------------|--------|---------|
| S1 | Security | No raw HTML from model output: disable HTML passthrough in the markdown renderer, or post-process with a restrictive allowlist — no `iframe`/`img`/`video`/`audio`/`object`/`embed`, no remote `src`. Link-scheme allowlist; `rel="noopener noreferrer"` on every model-authored link; **no automatic network fetch of any model-authored URL**. | I1, I6, I9 — and it is the **only** control on the remote-image path (see below) | **open** — PR #13 finding B5. Grafana's shared sanitizer is not this control: `packages/grafana-data/src/text/sanitize.ts` (v11.4.0) keeps `img`/`iframe`/`video`/`audio` in the js-xss allowlist, so the plugin must narrow it locally. | now |
