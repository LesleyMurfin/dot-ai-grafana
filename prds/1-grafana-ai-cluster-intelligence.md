...
- [x] **M7 — Map/Explore/show-me navigation surface.** In v1 scope, folded back from [PRD #3](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23) / [PR #22](https://github.com/LesleyMurfin/dot-ai-grafana/pull/22) — tracked as [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6). **State on `main` today:** Explore/Drilldown builders [#52](https://github.com/vfarcic/dot-ai-grafana/pull/52); Map + 0-hop show-me [#79](https://github.com/vfarcic/dot-ai-grafana/pull/79); firing-alert `dashboardUid` → `/d/<uid>` [#89](https://github.com/vfarcic/dot-ai-grafana/pull/89) (closes #81); sanitized GFM Answer [#51](https://github.com/vfarcic/dot-ai-grafana/pull/51) (Decision 12); #66 option 1 serve (`/alerting/list` + original 30-phrase show-me contract). Recommend closing #6. Never `GET /api/search` or `/api/datasources`.
...
### 2026-09-19 — #6 serve flip (alerts / dashboards)

- **Issue**: [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6) owns Map / Explore / show-me. #79 closed [#66](https://github.com/vfarcic/dot-ai-grafana/issues/66) by narrowing the 0-hop accept set (option 2) because `buildDrilldownLinks` had no alerts target and `dashboardUids` had no engine-independent source. Viktor recorded option 1 as the better product outcome and unblocked it after [#78](https://github.com/vfarcic/dot-ai-grafana/pull/78); [#89](https://github.com/vfarcic/dot-ai-grafana/pull/89) then carried firing-alert dashboard UIDs into Map.
- **Action**: serve the two held-back nouns. `buildDrilldownLinks` always emits `/alerting/list` (Grafana unified alerting; no search API). `isShowMeOnly` restores the original 30-phrase contract `(show me|open|display)( the)? (logs|alerts|traces|metrics|dashboards)`. Diagnosis tokens still force POST. M7 row above names the merged PRs and recommends closing #6.
- **Does not reopen**: #81 (closed by #89). No `GET /api/search`. No `/api/datasources`.
