...
As-built: M0–M9 cover v1 (M7 in scope and landed — see the M7 row). Floor is `grafanaDependency: ">=11.0.0"` (11.4 libs), not 10.x. See [As-built v1](#expansion-as-built-v1-this-contribution).
...
- [x] **M7 — Map/Explore/show-me navigation surface.** In v1 scope, folded back from [PRD #3](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23) / [PR #22](https://github.com/LesleyMurfin/dot-ai-grafana/pull/22) — tracked as [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6). **State on `main` today:** Explore/Drilldown builders [#52](https://github.com/vfarcic/dot-ai-grafana/pull/52); Map-panel wiring and the 0-hop classifier [#79](https://github.com/vfarcic/dot-ai-grafana/pull/79); firing-alert `dashboardUid` → `/d/<uid>` [#89](https://github.com/vfarcic/dot-ai-grafana/pull/89) (closes [#81](https://github.com/vfarcic/dot-ai-grafana/issues/81)). `show me the alerts|dashboards` now take the 0-hop path when Map can serve them (`/alerting/list`, or `dash-<uid>` from alert frames); dashboards still POST when no UID is present, which is the remaining #66 fail-safe. Checked for **scope** — M7 belongs to v1, not to PRD #3.
...
### 2026-09-19 — #66 option 1: serve show-me alerts / dashboards

- **Issue**: [#6](https://github.com/vfarcic/dot-ai-grafana/issues/6) still listed `show me the logs|alerts|traces|metrics|dashboards` as the navigation contract. [#79](https://github.com/vfarcic/dot-ai-grafana/pull/79) closed [#66](https://github.com/vfarcic/dot-ai-grafana/issues/66) with option 2 (narrow the accept set) because `buildDrilldownLinks` had no alerts link and `dashboardUids` had no engine-independent source. [#78](https://github.com/vfarcic/dot-ai-grafana/pull/78) and [#89](https://github.com/vfarcic/dot-ai-grafana/pull/89) later unblocked option 1; the classifier comments still cited closed #47.
- **Action**: restore the five-noun production (30 phrases). Emit `/alerting/list` from `buildDrilldownLinks`. 0-hop `alerts` / `dashboards` only when Map already has `alerting-list` or `dash-<uid>`; otherwise POST (the #66 fail-safe). Update the M7 "state on `main` today" clause to name #89.
- **Not changed**: Map stays above the Response (as shipped in #79 / `changelog.d/6.feature.md`). Never `GET /api/search` or `GET /api/datasources`.
