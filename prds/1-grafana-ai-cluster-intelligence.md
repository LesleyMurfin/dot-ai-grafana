...
|----|--------|---------|------------------|--------|---------|
| S1 | Security | **Drop the elements, do not validate their URLs.** No raw HTML from model output: disable HTML passthrough in the markdown renderer, or post-process with a restrictive allowlist — no `iframe`/`img`/`video`/`audio`/`object`/`embed`, no remote `src`. Link-scheme allowlist that also rejects **protocol-relative** (`//host/...`) URLs; `rel="noopener noreferrer"` on every model-authored link; **no automatic network fetch of any model-authored URL**. This is deliberately **stronger than CSA's recommendation** (allow-list the domains): domain validation is exactly what GrafanaGhost defeated, so we remove the capability instead of policing its argument — the CamoLeak fix shape. | I1, I6, I9 — and it is the **only** control on the remote-image path (see below) | **open** — PR #13 finding B5. Two implementation constraints: Grafana's shared sanitizer is not this control (`packages/grafana-data/src/text/sanitize.ts`, v11.4.0 keeps `img`/`iframe`/`video`/`audio`, and js-xss `safeAttrValue` accepts any `href`/`src` beginning with `/`, so `//host/px.png` passes), and `marked`'s `sanitize` option was removed in v8.0.0 — raw-HTML suppression must therefore be a renderer override on a **plugin-owned `new Marked()` instance**, since Grafana mutates the global singleton. | now |
...
| G1 | Governance | The trust boundary is documented **and testable**: an adversarial-telemetry corpus runs in CI against the rendering path. | I1, I6, I9 regression | **planned** — fixtures landed (`src/utils/__fixtures__/adversarialTelemetry.ts`); the consuming test lands with the S1 fix. See Milestones. | now |
...
**S1 is externally recommended, not just our opinion — and we go further.** The CSA note's own guidance, verbatim: "Where Markdown-based output rendering is enabled — particularly for image embeds — restrict to allow-listed domains and disallow protocol-relative URLs, the specific exfiltration channel exploited by GrafanaGhost." We adopt the protocol-relative prohibition as written, but **not** the domain allow-list as the primary control: a URL validator is the component GrafanaGhost bypassed, so S1 removes the embedding elements outright — the shape GitHub chose for CamoLeak — and the allow-list becomes at most a second line for links the operator explicitly clicks.
...
**Citation hygiene** (each of these errors would discredit the finding): CamoLeak is widely mis-attributed to **CVE-2025-59145**, which is an unrelated npm `color-name` malware advisory — cite CamoLeak by name, no CVE. GrafanaGhost has **no CVE either**; the two CVEs in Grafana's 2026-04-07 release — **CVE-2026-27876** (CVSS 9.1, arbitrary file write → RCE) and **CVE-2026-27880** (CVSS 7.5, unauthenticated DoS), fixed in 12.4.2 / 12.3.6 / 12.2.8 / 12.1.10 / 11.6.14 — are adjacent and unrelated to the prompt-injection finding; never attach those numbers to it. By contrast, EchoLeak (Microsoft 365 Copilot, zero-click LLM exfiltration) **does** carry a verified identifier, **CVE-2025-32711** — so "same class, no CVE" is a property of these two cases, not of the class.
...
| 2026-09-04 | **The telemetry channel is demonstrated, not theoretical** — and S1 goes further than the published recommendation: drop the embedding elements rather than allow-list their domains | **GrafanaGhost** (Noma Security, disclosed 2026-04-07, patched) is indirect prompt injection into **Grafana's own** AI assistant with the injection channel in **log content**: CSA AI Safety Initiative, *Indirect Prompt Injection Goes Operational* (2026-04-26) — "poison Grafana log entries with carefully crafted query parameters … exfiltration channel exploited Markdown image rendering with protocol-relative URLs (paths beginning with //), which passed Grafana's URL validation"; the OWASP GenAI Exploit Round-up Q1 2026 (2026-04-14) classifies it LLM01/LLM02/LLM05 · ASI01/ASI02/ASI09, "Hidden instructions in logs caused data exfiltration via Markdown". **LogJack** ([arXiv:2604.15368](https://arxiv.org/abs/2604.15368), 2026-04-15) benchmarks log-embedded payloads across 8 models: verbatim command execution 0%–86.2%, one model still at 30% under passive "do not execute" instructions, provider guardrails largely blind — so **model choice is not a control**. Because a URL validator is precisely what GrafanaGhost defeated, S1 removes `img`/`iframe`/`video`/`audio`/`object`/`embed` outright (the CamoLeak fix shape) instead of adopting CSA's domain allow-list as the primary control. Still narrowly true and recorded as such: MITRE ATLAS has catalogued 0 of 57, and `grafana/mcp-grafana#680` is unanswered since 2026-03-24. Hygiene: GrafanaGhost and CamoLeak have **no CVE** (CVE-2026-27876/27880 are unrelated same-release Grafana bugs; CVE-2025-59145 is an npm advisory); EchoLeak's CVE-2025-32711 is verified |
