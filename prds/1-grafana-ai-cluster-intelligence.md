...
| I12 | **Reviewer/CI tooling trusting the artifact it reviews.** A different surface than I1–I11: not the model's *output* reaching an operator, but this repository's own *inbound* code/PR content reaching review or CI tooling with reviewer-level trust — a script executed from the PR's own checkout, or instructions embedded in a PR title, body, comment, or file treated as directives rather than data. Not yet tied to a specific line in this codebase (no custom review/CI script here executes PR-sourced code today, as far as checked) — recorded because the *class* has live, named precedent, not because a concrete instance was found here. | Same blind spot as I1–I6 (untrusted content, privileged consumer) but the consumer is tooling/CI, not the operator's browser or the model — none of S1–S4/C1/C2 govern this inbound path at all. **CVE-2025-66032** / [GHSA-xq4m-mc3c-vvg3](https://github.com/advisories/GHSA-xq4m-mc3c-vvg3) (CVSS 8.7, CWE-20 + CWE-77): one crafted PR/issue disclosure hijacked three AI coding-agent platforms at once — Anthropic's Claude Code (GitHub Action + Security Review action), Google's Gemini CLI Action, and GitHub's Copilot Coding Agent — via unsanitized PR titles, crafted issue bodies, and hidden HTML comments, exfiltrating repository secrets through GitHub itself. Maps to OWASP Top 10 for LLM Applications 2026, **Excessive Agency** (#3, up from #6 in 2025). |
...
| S5 | Security | Review/CI tooling (risk assessors, validators, linters, any script the review process runs) is sourced from a pinned, trusted location outside the artifact under review — never executed from the PR/branch/commit being reviewed. Instructions embedded in PR titles, bodies, comments, or file content are inert data, never directives, regardless of phrasing. | I12 | **open** — no custom review/CI script in this repo currently executes PR-sourced code, so there is nothing to fix in this codebase today; recorded as a standing design constraint for any future review/CI automation (including AI-driven review tooling), not a remediation of an existing gap. | next — binding before any custom review/CI automation is added, not before this PR |
