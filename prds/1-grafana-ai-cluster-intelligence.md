...
The plugin provides a simple text-based interface: tool selector, text input for the intent, and an answer area displaying the agent's response. ~~No rich visualizations (Mermaid, cards, tables) — just plain text responses.~~ **Amended 2026-09-04 — superseded in part, see [Decision 12](#expansion-design-decisions):** only the *rendering* half of that sentence is retired. The answer is rendered as **sanitized GFM markdown** (headings, lists, tables, code, links), so "no tables" no longer holds; Mermaid diagrams, cards and client-side charts stay out of scope. The *request* half stands unchanged — the plugin sends the **plain** intent and **never** prefixes `[visualization]` ([tool → endpoint map](#expansion-tool--endpoint-map)), so dot-ai stays in plain-answer mode and the plugin only renders markdown the model already emits. Measured at `efd80a4`: `main` renders the answer inside a `<pre>` block (`src/pages/DotAIPage.tsx`). The rendering code lands in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13).
...
| Tool | Render in the answer | Ignore (never rendered) |
|------|----------------------|-------------------------|
...
*Header amended 2026-09-04 — was `| Tool | Render as text | Ignore in the text-only UI |`.* The answer is rendered as sanitized markdown rather than as a plain-text blob ([Decision 12](#expansion-design-decisions)). The field mapping itself is unchanged, and `visualizationUrl` stays in the ignore column: the plugin never asks dot-ai for visualization mode.
...
12. **Text-only rendering retired — rendering half only; navigation extras return to the first release (tracked in [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)).** *Amended 2026-09-04: this row previously recorded only "navigation extras deferred out of the first release, now returning", which understated the decision actually being made.* The project's original **text-only** decision — `CLAUDE.md` **Key Design Decisions**, [Solution Overview](#solution-overview) ("No rich visualizations (Mermaid, cards, tables) — just plain text responses") and the old `Ignore in the text-only UI` table header — is **superseded in its rendering half**: the answer is rendered as sanitized GFM markdown through Grafana's own sanitizer (`renderMarkdown` from `@grafana/data`), and the Answer area carries outbound Explore / Drilldown / firing-alert dashboard links. **The request half is not superseded and must stay that way:** the plugin still POSTs the **plain** intent and still **does not** prefix `[visualization]` ([tool → endpoint map](#expansion-tool--endpoint-map), Decision 1 above), so dot-ai stays in plain-answer mode — the plugin only renders markdown the model already emits. Reading this amendment as permission to turn `[visualization]` on would be a misreading of it. Still out of scope either way: Mermaid diagrams, cards, client-side charts, and dot-ai's visualization mode. Navigation extras (Map / Explore / show-me / markdown answer) were built, then pulled out of the v1 PR after review so that review stayed scoped to analysis-only packing; the decision here is that they land on **PRD #1** rather than a second product PRD. **This is a decision record, not shipped behaviour:** measured at `efd80a4`, `main` renders the answer as `<pre>{responseText}</pre>` in `src/pages/DotAIPage.tsx`. The code lands in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13) — sanitized markdown answers, Explore panes + Drilldown app links, firing-alert `dashboardUid` to `/d/<uid>` links (never `GET /api/search`), and the show-me 0-hop classifier (complete-phrase match; diagnosis tokens force the engine path).

13. **Usability items are first-release scope carried forward (tracked in [issue #8](https://github.com/vfarcic/dot-ai-grafana/issues/8)), not a separate product document.** Thread integrity, honest multi-hop progress, and shipping docs/compatibility matrix extend the existing error-handling/loading and documentation milestones rather than opening a second PRD. Progress and docs work stay on PRD #1.
...
| Deferred | **GitOps execute** → [PRD #2](https://github.com/LesleyMurfin/dot-ai-grafana/issues/13) / [PR #18](https://github.com/LesleyMurfin/dot-ai-grafana/pull/18). Also: async 202; generated OpenAPI client; Grafana.com signing. **Reassigned, not shipped:** M7 Map/Explore/show-me/markdown navigation is decided for PRD #1 ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)), so [PRD #3](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23) / [PR #22](https://github.com/LesleyMurfin/dot-ai-grafana/pull/22) (0.2.x) no longer own it — the code is absent from `main` at `efd80a4`; it lands in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13) |
...
**Send Grafana evidence** (`jsonData.sendGrafanaEvidence`, default on; missing/undefined = send) is independent of **Show context**. When send is off, Asks do not pack Grafana DS facts (`fetchStackContext` / `loadStack` not called). No datasource UID pickers: types discovered via `getDataSourceSrv().getList({ type })`. Per-type checkboxes are future. Related alerts are already in **Current** from Alertmanager when send is on. Dashboard-to-open / Explore / show-me / markdown Answer are **decided for this PRD** ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)) but are **absent from `main` at `efd80a4`** — they land in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13); dashboard links come from firing-alert `dashboardUid`. Grafana `/apis` dashboard inventory is still later — this plugin never calls `GET /api/search`.
...
| **Security** | Partial | Token in `secureJsonData` (backend only); remediate allowlist (no execute); 401/403 → 502 no secret leak; no generated OpenAPI client; Debug Log off by default; public `http` rejected | `http` only for loopback, RFC1918, or in-cluster DNS (`*.svc` / `*.cluster.local`); ~~shared Bearer (any user who can open the plugin)~~ → **amended 2026-09-04:** shared Bearer, reachable by **any Editor-or-above org user** — the tool routes gate on `isEditorOrAbove` and Viewer, empty, unknown and `nil` users are refused before any upstream dial (`pkg/plugin/resources.go` at `efd80a4`: the checks in `handleQuery` / `handleRemediate`, the fail-closed switch in `isEditorOrAbove`). The token is still shared, so the gap is authorization granularity, not open access; per-user identity upstream remains out of scope ([#25](https://github.com/vfarcic/dot-ai-grafana/pull/25) / [#26](https://github.com/vfarcic/dot-ai-grafana/issues/26)); Current/logs go to the LLM when send is on |
...
- Rich visualizations: Mermaid diagrams, cards, client-side charts, and dot-ai's `[visualization]` mode. *Amended 2026-09-04 ([Decision 12](#expansion-design-decisions)):* rendering the answer as sanitized GFM markdown — including fenced code blocks (styled, **not** syntax-highlighted) and tables — is no longer out of scope; asking dot-ai for rich output still is.
...
> **SSOT for milestone completion.** Work Log (below) is evidence only and must not be read as a second checklist. Checkbox completion state is authoritative solely under this Milestones section (including the original checklist and any `### Expansion:` milestone detail).
...
- [x] **Error handling and loading states** — Connection errors, auth failures, timeouts displayed clearly; loading spinner during requests; Cancel + Retry in v1. **Carry-forward ([issue #8](https://github.com/vfarcic/dot-ai-grafana/issues/8)):** honest stage progress on multi-hop / long remediate; cancel/retry remain usable.
- [x] **Documentation and installation guide** — README with setup instructions, configuration guide, and screenshots. **Carry-forward ([issue #8](https://github.com/vfarcic/dot-ai-grafana/issues/8)):** screenshots, Grafana/plugin compatibility matrix, thread-integrity operator notes; canonical plugin id `devopstoolkit-dotai-app`.
- [x] **Grafana version compatibility testing** — CI Playwright on Grafana **11.0–13 + nightly** (floor `>=11.0.0`, 11.4 libs). Not 9.x/10.x.
- [ ] **Navigation surface return (Map / Explore / show-me)** ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)) — Sanitized markdown answers; Explore panes + Drilldown app links when installed; firing-alert `dashboardUid` to dashboard links (**never** `GET /api/search` or `/api/datasources`); show-me complete-phrase 0-hop classifier with diagnosis-wins. Empty dashboard UID → no dashboard link, Ask still succeeds. *Scope decided ([Decision 12](#expansion-design-decisions)); unchecked until the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13) merge; at `efd80a4`, `main` renders the answer in a `<pre>` block.*
- [ ] **Elevated-role gate on tool resource routes** ([#26](https://github.com/vfarcic/dot-ai-grafana/issues/26) / [#25](https://github.com/vfarcic/dot-ai-grafana/pull/25)) — Tool routes require elevated Grafana org role; deny before any upstream engine call; fail closed with no user; Test connection remains Admin-only. *Unchecked until #25 merges.*
- [ ] **Usability carry-forward — remaining thread/progress/docs** ([issue #8](https://github.com/vfarcic/dot-ai-grafana/issues/8)) — Honest multi-hop progress UX and shipping screenshots/compatibility matrix still open. Explicit Clear thread is present. *Done when an operator can follow multi-hop progress clearly and docs/screenshots ship.*


As-built: M0–M9 cover v1, M7 navigation included ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)). Floor is `grafanaDependency: ">=11.0.0"` (11.4 libs), not 10.x. See [As-built v1](#expansion-as-built-v1-this-contribution).
...
- [ ] **M7 — Dashboard deep-link.** **Decided for this PRD; absent from `main` at `efd80a4`** — ownership moved here via [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6); [PRD #3](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23) / [PR #22](https://github.com/LesleyMurfin/dot-ai-grafana/pull/22) (plugin 0.2.x) no longer own it. Alert-frame `dashboardUid` → `/d/<uid>`, Explore panes + Drilldown app links when installed; **never** `GET /api/search`. *Unchecked until the navigation feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13) merge.*
...
```
  PRD #1  this file / vfarcic#3 / fork #21     v1 0.1.0   analysis-only pack (shipped) + Map / Explore / show-me (decided, PRs open)
  PRD #2  issue #13 / fork #18                  post-v1    GitOps PR execute
  PRD #3  issue #23 / fork #22                  folded     Map / Explore / show-me → PRD #1 (issue #6)
```

| PRD | GitHub | Owns | Does not own |
|---|---|---|---|
| **#1** (this file) | [vfarcic#3](https://github.com/vfarcic/dot-ai-grafana/pull/3) · [fork #21](https://github.com/LesleyMurfin/dot-ai-grafana/pull/21) | Query + analysis-only remediate, Current packing, 0.1.0 (in `main`); Map `/d/uid`, Explore/Drilldown, show-me skip POST, markdown Answer ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6)) — owned here, code still in review | GitOps execute |
...
| **#3** | [issue #23](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23) · [PR #22](https://github.com/LesleyMurfin/dot-ai-grafana/pull/22) | *(nothing — Map `/d/<uid>`, Explore/Drilldown, show-me skip POST, markdown Answer folded into PRD #1 via [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6))* | GitOps execute, v1 0.1.0 |
...
| *(new in this revision)* | **M7** dashboard→intent deep-link — **In v1**; [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6) |
...
9. **Navigation surface ([issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6))** — Confirm Drilldown app ids stay installed-apps probe (not hard-coded only) and the show-me diagnosis-wins token list.
10. **Usability carry-forward ([issue #8](https://github.com/vfarcic/dot-ai-grafana/issues/8))** — Remaining multi-hop progress chrome and screenshots/matrix: land on this PRD vs a thin follow-up?
...
|----------|------|-----------|--------|-------------|-------|
| Plugin tool resource routes require an elevated Grafana org role (Editor or above); denial happens before any upstream engine call; requests with no user fail closed; existing Admin gate on Test connection is unchanged | 2026-09-03 | Raised as a non-blocking observation during review of the merged plugin PR; being landed separately rather than folded into that merge | Operators need elevated role to use Ask; lower roles see a clear denial | Code in [#25](https://github.com/vfarcic/dot-ai-grafana/pull/25); tracked by [#26](https://github.com/vfarcic/dot-ai-grafana/issues/26) | Maintainer + contributor |
| Return navigation surface pulled from v1 review: markdown answers, Explore/Drilldown links, alert dashboard links, show-me 0-hop classifier | 2026-09-03 | Features were built then pulled to keep the v1 PR scoped; issue #6 tracks them; returning them completes first-release navigation without `/api/search` | Operators get Map links and pure show-me navigation without an engine hop | `ResponseMarkdown`, `grafanaExplore` (`isShowMeOnly`, Explore/Drilldown URLs), dashboard UIDs from alert frames, orchestrator 0-hop path — *amended 2026-09-04: none of this is in `main`; the code lands in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13)* | Maintainer + contributor |
| Navigation extras and usability carry-forwards stay on PRD #1 (issues #6, #8) rather than a second product PRD | 2026-09-03 | Same first-release scope; separate PRDs would split the thin client story | Trackers remain #6 and #8 | PRD milestone + Decision Log only for scope; code as above | Maintainer + contributor |
| Text-only response **rendering** decision superseded: answers render as sanitized GFM markdown (headings, lists, tables, code, links). The **request** half is *not* superseded — the plugin keeps sending the plain intent and still never prefixes `[visualization]` | 2026-09-04 | The original text-only decision (`CLAUDE.md` **Key Design Decisions**, [Solution Overview](#solution-overview), and the old `Ignore in the text-only UI` table header) asserted the opposite of where the product is going, and recording it as "navigation extras returning" understated the decision (review of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13), B4). Only the rendering half changed: the plugin renders markdown dot-ai already emits, it does not ask for rich output | Operators read a formatted answer instead of a `<pre>` blob; "no tables" is no longer binding on rendering; Mermaid, cards, client-side charts and dot-ai's `[visualization]` mode stay out of scope; a contributor reading the text-only bullet is no longer misled into thinking plain text is still policy — or into turning `[visualization]` on | **Decision only in this PR.** Measured at `efd80a4`: `main` renders `<pre>{responseText}</pre>` (`src/pages/DotAIPage.tsx`). The rendering code (`ResponseMarkdown` over `renderMarkdown` from `@grafana/data`) lands in the feature PRs split out of [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13) | Maintainer + contributor |
...
### 2026-09-03 — navigation fold: one story

- **Issue**: Decision 12, the M7 milestone and the Decision Log said Map / Explore / show-me / markdown ship in this PR, while the As-built Deferred row, the M7 roadmap entry, the Send-Grafana-evidence paragraph and the related-PRD table still deferred them to [PRD #3](https://github.com/LesleyMurfin/dot-ai-grafana/issues/23).
- **Action**: Deferred row, M7 milestone, as-built summary, evidence paragraph, release ladder and related-PRD rows now all read as shipped here under [issue #6](https://github.com/vfarcic/dot-ai-grafana/issues/6); PRD #3 rows kept and marked folded.
- **Prompt**: keep the PRD self-consistent on PR #13 (not a new PR).

### 2026-09-04 — PRD split out of PR #13; text-only rendering decision retired

- **Issue**: The maintainer asked for the PRD documentation to land as its own PR (review on [#13](https://github.com/vfarcic/dot-ai-grafana/pull/13)) and blocked it on B4: Decision 12 recorded "navigation extras returning" while the project's text-only decision was still asserted verbatim in `CLAUDE.md` **Key Design Decisions**, in [Solution Overview](#solution-overview), in the `Ignore in the text-only UI` table header and in the out-of-scope list. The doc also read as though navigation had shipped, which is not true of `main`.
- **Action**: Decision 12 rewritten to record the **rendering** half of the text-only decision as superseded, with the request half (`[visualization]` never prefixed) explicitly preserved; Decision Log row added; Solution Overview sentence, presentation-table header and out-of-scope bullet amended in place rather than deleted; navigation milestone, M7, the as-built Deferred row, the Send-Grafana-evidence paragraph and the related-PRD rows re-stated as decided-but-not-merged, since `main` renders `<pre>{responseText}</pre>`.
- **Prompt**: land the PRD half of #13 as its own PR against `main`, with B4 addressed.
- **Also corrected**: the By-Design risk register's **Security** Gap cell still read "shared Bearer (any user who can open the plugin)", false since [#25](https://github.com/vfarcic/dot-ai-grafana/pull/25) gated the tool routes on `isEditorOrAbove`. Amended in place to "any Editor-or-above org user", anchored to `pkg/plugin/resources.go` at `efd80a4`. Found by a claim-surface check, not by reading: `7e3ab39` corrected the same claim in `CLAUDE.md` and `README.md` and missed this third surface. The **Consent** gap one row below — "Grafana users share one token; no per-user consent / OAuth" — is still true at `efd80a4` and is deliberately left alone.
