...
The plugin provides a simple interface: tool selector, text input for the intent, and a response area displaying the agent's response. **Amendment (2026-09-05, Design Decision 12):** the original text-only decision is retired for *rendering* only, and only once the renderer lands — on `main` that response area is still a plain-text area, and **PR #51** is what makes it render the model's own GFM markdown (headings, lists, tables, code blocks, links) as sanitized HTML, code blocks unhighlighted. The *request* half stands unchanged and holds today: the plugin never requests rich visualizations from dot-ai (no `[visualization]` prefix; see the tool→endpoint map below and Design Decision 1). This amendment is accurate as written while #51 is open and does not wait for it; **#51 carries the flip to the present tense** (Decision 12).
...
12. **Markdown rendering vs. text-only — RESOLVED, amends Decision 1; effective with PR #51.** The original text-only decision banned both *requesting* and *rendering* rich visualizations. Only the rendering half is retired, and only once the renderer lands. **State on `main` today:** the response area is a plain-text area — `src/pages/DotAIPage.tsx` renders `<pre className={styles.pre}>{responseText}</pre>`, there is no `src/components/ResponseMarkdown.tsx`, and `package.json` carries no markdown or sanitizer dependency. **PR #51** adds that component and swaps the `<pre>` for it; from then on the response area renders the model's own GFM markdown (headings, lists, tables, code blocks, links) as sanitized HTML, with code blocks **unhighlighted** — the sanitizer strips `class` (including marked's `language-*`) and no highlighter is added. **Merge order — this decision does *not* wait for #51.** As worded above it describes `main` accurately while #51 is open, so landing it first is correct; the earlier "must merge after #51" requirement was written for an earlier revision that used the present indicative and is retired with it. The obligation is now on **#51**: the same commit that adds the renderer must flip the "state on `main` today" clauses here, at line 57, in the UX-states success step, the out-of-scope bullet, the M4 row, the original-section comparison table, `README.md` and `CLAUDE.md` to the present tense. Merging #51 without that flip inverts this defect instead of fixing it. The request half is unchanged and holds today — the plugin never prefixes `[visualization]` to the intent (Decision 1); dot-ai is never asked to switch into rich-visualization mode. **Caveat:** that surviving half holds largely by *absence* rather than by an enforced control. It is not wholly unguarded — `src/utils/progressiveContext.test.ts` asserts the first-turn packing by exact equality (`expect(text).toBe(...)`), so a `[visualization]` prefix introduced inside `pack()` would fail today. The uncovered path is `stablePreamble()` itself: that assertion interpolates `stablePreamble`, so it is self-referential, and the `stablePreamble` tests only use `toMatch`. A prefix added there passes both. A one-line assertion that `stablePreamble()`'s output — and the packed text — contain no `[visualization]` would close it; out of scope for this documentation-only change.
...
5. **Success**: answer displayed as plain text, rendered as sanitized markdown once PR #51 lands (Decision 12); **Copy** on the whole response and per recommended `command`; a "Show raw response" toggle.
...
- Rich visualizations *requested from dot-ai* (Mermaid diagrams, cards, charts) — the plugin never prefixes `[visualization]` to the intent (Decision 1). Rendering the model's own GFM markdown as sanitized HTML comes **in** scope once **PR #51** lands (Decision 12 amends this line for rendering only); until then the response area is plain text. Syntax highlighting stays out of scope either way: code blocks render unhighlighted, since the sanitizer strips the `language-*` class and no highlighter ships.
...
- [x] **M4 — Query UI.** Shipped the `summary` response area as **plain text**, and it is still plain text on `main`; sanitized-markdown rendering of that same `summary` field arrives with PR #51 (Decision 12), not with M4. Grafana DS **Current/Map** packed into `{intent}`; condensed **Prior:** from recent turns (≤240 chars inside 1000-char budget). Full History on screen. No cluster-context chip, raw-response toggle, or char counter in v1.
...
| **Solution Overview** | [Solution Overview](#solution-overview) | Same: Query + Remediate analysis-only; presentation still plain text on `main`, amended to sanitized markdown once PR #51 lands (Decision 12) |
