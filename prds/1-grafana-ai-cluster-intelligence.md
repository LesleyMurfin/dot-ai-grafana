...
- [x] **M4 — Query UI.** Shipped the `summary` response area as **plain text**; sanitized-markdown rendering of that same `summary` field landed later in PR #51 (Decision 12), not with M4. Grafana DS **Current/Map** packed into `{intent}`; History display-only. No cluster-context chip, raw-response toggle, or char counter in v1.
