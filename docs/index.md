...
| Send Grafana evidence | On | When on, **Query** Asks that read live data replace Current with fresh Grafana datasource facts and the page shows a consent info Alert naming them (Loki, Prometheus, Tempo, Alertmanager); missing/undefined = send. **Remediate never reads a datasource, on or off.** Independent of Show context. |
